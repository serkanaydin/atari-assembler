...
synent(){
	int cpc, //the current program counter, which
                 //holds the address of the next pseudo-instruction in the Syntax Tables.
            spc, 
            six,   //The Syntax Stack starts at $480 at the label SIX.
            sox, 
            stklvl;

	cpc=assembly[srcadr];   //assembly array contains the syntax code.
	spc=assembly[srcadr];
	...
	while(){
		...
		if (code == 5)         //tnvar
			if (!tvar(0x0))        //returns 1 if succesful.
				if ( fail() )
					return(1);
		else if (code == 7 )  //tsvar
			if (!tvar(0x80)) 
				if ( fail() )
					return(1);
		else if (code == 4) //tncon
			if (!tncon()) 
				if ( fail() )
					return(1);
		else if (code ==6)  //tscon
			if (!tscon())         
				if ( fail() )
					return(1);				
		else if (code == 10) //eif
			...
			code=nxsc();            //read the address to go..
			if (stklvl+4 == 0)
				goto erltl;
			stack[stklvl+1]=cix;
			stack[stklvl+2]=cox;
		...
		else if ((code == 2)||(code == 3)){   //or=3 veya return=3
			if (stklvl == 0)
				return(0);     <======== main exit of syntaxer
			//if we are here, we hit an OR or RET without an error, 
                        //ie, without calling fail(). This means we are finished 
                        //with the current line. But, as the stack is still full, 
                        //we still have not finished the syntax code. Hence we have to 
                        //return and continue from where we left.
...
fail() {
	while (1)
		//skip till you find a RET=2 or an OR=3.	
		code=nxsc();         //Increments cpc and gets next codeword.
        	while (code<2 || code >3 ){        
			if (code == 0 || code==1 ){         
				inccpc();
				inccpc();
		...
		//An OR=3 is encountered.We must restore inbuff and outbuff and try again.
			...
			if (cix>maxcix)    //here code=2.
				maxcix = cix;	//indicates the displacement into LBUFF of the error. 
				                //The character at this location will 
			 	                //then be displayed in inverse video
	                //the code restores CIX and COX to their before-failure values and 
	                //seturns to synent() to try this new OR choice.
	                //cix ve cox syntax stack'tan restore ediliyorlar...
			cix = stack[stklvl-3]; 
			cox = stack[stklvl-2]; 
			return(0)    //return to sy
		}   //returns to the start of the while loop in synent.	
	}	
}
